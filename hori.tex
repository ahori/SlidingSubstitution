...
communication performance following the substitution. It will be shown
that when a failure occurs, the peer-to-peer (P2P)
communication performance on the K computer can be slowed by a factor
of three and collective performance can be cut in half. On
BG/Q, P2P performance can be slowed by a factor of five and
collective performance can be slowed by a factor of ten. 
However, those numbers can be reduced by using an appropriate
substitution method.
...
\item some behaviour and characteristics of proposed spare node
  allocation and substitution methods are revealed by simulations and
  evaluations on the K computer, BG/Q and TSUBAME 2.5,
\end{itemize}
...
For simplicity, we will consider only 2D networks with XY routing
routing\citep{Zhang:2009:CRX:1603897.1605067}.
...
(sub)optimal node-rank mapping will be very important.
...
When replacing a failed node with a spare node, the spare node must be
integrated into the computations. In 1D or 2D sliding, the
computations on these nodes are migrated. We are planning to develop a
framework to hide the complexity of allocating and utilizing spare
nodes. The technical issues and actual overhead cost of doing so will
be reported when the development is completed.
