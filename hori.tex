...
%%\newcounter{commentAH}
%%\newcommand{\AH}[1]{%
%%  \refstepcounter{commentAH}%
%%  \textcolor{blue}{$\langle\langle$\thecommentAH$\rangle\rangle$ {#1}}}%
...
\comment{(see the sample program codelet in Figure \ref{prg:1d-prog})}
...
\AH{
\[
S_r^i = L_{substitution}^i / L_{no_substitution}
\]
}
\AH{
Here, $L_{substitution}^i$ is the measured communication latency after
$i$th substitution, $L_{no_substitution}$ is the latency without having any 
node failure but having spare node set, andthe $S_r^i$ is the
slowdown ratio. 
}
...
speed of application execution running as the shadow replica can be
controlled by using  DVFS. In HPC, unlike cloud computing,
applications are supposed to run as fast as they can. Having any sort
of replication process(es) consumes twice hardware resource.  In our
proposed spare node utilization model, the additional resource for the
fault mitigation is far less than twice as shown in Figure
\ref{fig:sparenode-percentage}. However, the time to care SLA at
supercomputing centers may come in the future. 
}
